...
@Injectable()
export class MockAdapter implements ICourierAdapter {
  private readonly logger = new Logger(MockAdapter.name);

  async createOrder(dto: CreateOrderDto): Promise<NormalizedOrderResponse> {
    this.logger.log(`[MOCK] Creating order: ${dto.order_id}`);

    await this.sleep(200);

    const awb = `MOCK-AWB-${uuidv4().slice(0, 8).toUpperCase()}`;
    const raw = {
      status: 'success',
      awbNumber: awb,
      shipmentId: `MOCK-SHIP-${uuidv4().slice(0, 6)}`,
  ...
  async trackOrder(awbNumber: string): Promise<NormalizedTrackResponse> {
    this.logger.log(`[MOCK] Tracking order: ${awbNumber}`);
  ...
  async cancelOrder(awbNumber: string): Promise<NormalizedCancelResponse> {
    this.logger.log(`[MOCK] Cancelling order: ${awbNumber}`);

    await this.sleep(100);

    const raw = {
      status: 'success',
      message: `Mock order ${awbNumber} cancelled successfully`,
    };
